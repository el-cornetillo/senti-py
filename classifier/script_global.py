...
def ingest(path=None, save=False, verbose=True):
    if save and path is None:
        ...
        + ['no ' + e.strip() for e in open(pathsData['MERCADOLIBRE_NEG'], encoding='utf8').readlines() if \
        ...
        + ['no ' + e.strip() for e in open(pathsData['MERCADOLIBRE_POS'], encoding='utf8').readlines() if \
    ...
    if 'muy ' in x or ' re ' in x:
      ...
      #          return (score+(t-1)*importantScore)/t
                while ((decayRate-t)*(score + t*importantScore)/decayRate-0.5)*(importantScore-0.5)<0:
                    t = t + 1
                return ((decayRate-t-1)*score+(t+1)*importantScore)/decayRate
            else:
                return score
        else:
            return score
    ...
    if len(processPero(x))>1:
        ...
        if (preScore-0.5)*(postScore-0.5)<0:
            ...
            while (((decayRate-t)*preScore + t*postScore)/decayRate-0.5)*(postScore-0.5)<0:
                t = t + 1
            return ((decayRate-t-1)*preScore + (t+1)*postScore)/decayRate
        else:
...
class sentimentClassifier:
    def __init__(self, ngram_range = best_params['v__ngram_range'], max_features = best_params['v__max_features'],
        ...
        if self.lightModel:
            ...
            ('v', MarisaTfidfVectorizer(TfidfVectorizer(ngram_range=ngram_range, \
