...
def getComments(htmlUrl):
    ...
    def getCommentsOfPage(htmlUrl, pageNumber, commentsList, ratingsList):
        ...
        htmlContent = urlopen(req).read()
        soup = bs4.BeautifulSoup(htmlContent, 'html5lib')
        commentsOnPage = [elem.text.replace(Tab,'') for elem in soup.findAll('p', attrs={'itemprop': 'description'})]
        commentsList += commentsOnPage

        ratingsList += [float(elem.text) for elem in soup.findAll('i', attrs={'class': 'rating-points'})]
        
        return len(commentsOnPage)
    ...
    for pageIndex in pageIndexes:
        ...
        for restaurantUrl in restaurantsList:
            try:
                ...
                if totalReviews>=maxChunkSize:
                    df = pd.DataFrame(data = commentsData, columns = ['Comment', 'Rating'])
                    df.to_csv(str('./PedidosYaData'+('%d.csv' % iControl)), index=False, sep=';')
                    if verbose:
                        print('Fetched data from restaurant %d of 50' % counter)
                        totalReviewsControl += totalReviews
                        print('--------------DataSet now has %d reviews' % (totalReviewsControl))
                        counter = counter + 1
                    global commentsData
                    commentsData=[]
                    iControl = iControl + 1
